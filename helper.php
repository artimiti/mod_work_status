...
class ModWorkStatusHelper
{
    ...
    public static function getScheduleHtml($params)
    {
        ...
        $shouldMarker = (bool)$params->get('marker_current_day', 0);
        $shouldHighlight = (bool)$params->get('highlight_current_day', 0);
        $days = [
        ...
        foreach ($days as $dayIndex => $day) {
            $note = trim($day['note'] ?? '');
            $isCurrentDay = ((int)$dayIndex === $currentDayIndex);
            ...
            // В модалке классы и маркер должны зависеть от выбранных переключателей:
            // - marker_current_day -> добавляет uk-heading-bullet текущему дню
            // - highlight_current_day -> добавляет uk-text-success/uk-text-danger текущему дню
            $headingBulletClass = ($shouldMarker && $isCurrentDay) ? ' uk-heading-bullet' : '';
            $highlightClass = ($shouldHighlight && $isCurrentDay) ? ($isOpen ? 'uk-text-success' : 'uk-text-danger') : '';

            if ($note !== '') {
                // Разметка для дня с примечанием
                $html .= '<li class="el-item">';
                $html .= '<div class="uk-grid uk-grid-small uk-grid-stack@m' . ($highlightClass ? ' ' . $highlightClass : '') . '" uk-grid="">';
                $html .= '<div class="uk-width-expand@m"><div class="el-title uk-margin-remove' . $headingBulletClass . '">' . htmlspecialchars($day['label'], ENT_QUOTES, 'UTF-8') . '</div></div>';
                ...
                $timeText = $isOpen
                    ...
                    : 'Выходной';
                $html .= '<li class="el-item">';
                // Для текущего дня, когда включен хотя бы один переключатель, добавляем классы,
                // чтобы разметка в модальном окне соответствовала ожидаемой.
                if ($isCurrentDay && ($headingBulletClass !== '' || $highlightClass !== '')) {
                    $outerClass = 'uk-child-width-auto uk-grid-small';
                    $firstColumnClass = 'uk-width-expand';

                    // marker only
                    if ($headingBulletClass !== '' && $highlightClass === '') {
                        $outerClass .= ' uk-grid';
                        $firstColumnClass = 'uk-width-expand uk-first-column';
                    // highlight only
                    } elseif ($headingBulletClass === '' && $highlightClass !== '') {
                        $outerClass = 'uk-child-width-auto uk-grid uk-grid-small ' . $highlightClass;
                        $firstColumnClass = 'uk-first-column uk-width-expand';
                    // both
                    } elseif ($headingBulletClass !== '' && $highlightClass !== '') {
                        $outerClass .= ' ' . $highlightClass . ' uk-grid';
                        $firstColumnClass = 'uk-width-expand uk-first-column';
                    }

                    $html .= '<div class="' . $outerClass . '" uk-grid>';
                    $html .= '<div class="' . $firstColumnClass . '"><div class="el-title uk-margin-remove' . $headingBulletClass . '">' . htmlspecialchars($day['label'], ENT_QUOTES, 'UTF-8') . '</div></div>';
                } else {
                    $html .= '<div class="uk-child-width-auto uk-grid-small' . ($highlightClass ? ' ' . $highlightClass : '') . '" uk-grid>';
                    $html .= '<div class="uk-width-expand"><div class="el-title uk-margin-remove' . $headingBulletClass . '">' . htmlspecialchars($day['label'], ENT_QUOTES, 'UTF-8') . '</div></div>';
                }
