...
$uniqueId = 'work-status-title-modal-' . $module->id;

// Если есть объявление — всегда простой вывод, без модалки
if ($status['is_announcement']) {
    echo '<div class="work-status-title ' . $status['class'] . '">' .
...
<div class="work-status-title work-status-title--modal <?= $status['class']; ?>" uk-toggle="target: #<?= $uniqueId; ?>">
...
<div id="<?= $uniqueId; ?>" uk-modal>
    <div class="uk-modal-dialog uk-modal-body uk-margin-auto uk-margin-auto-vertical uk-border-rounded work-status-modal">
...
<?php else: ?>
<!-- Простой вывод -->
<div class="work-status-title <?= $status['class']; ?>">
